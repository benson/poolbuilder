...
import { fetchWithRetry, fetchAllSetCards, generateSealedPoolFromBoosterData, getDailySeed } from './mtg.js';
...
async function main() {
  ...
  const cards = await fetchAllSetCards(dailySet.code);
  console.log(`fetched ${cards.length} cards`);

  // Generate pool
  const pool = await generateSealedPoolFromBoosterData(dailySet.code, cards, 6, seed);
